...
## תזכורות במייל (Gmail SMTP + Vercel Cron)

האפליקציה תומכת בתזכורות אוטומטיות במייל למועדים קרובים. כל משתמש מגדיר את ה-Gmail שלו בתוך האפליקציה (דף הגדרות → סקציית Gmail), וכל תזכורת נשלחת דרך החשבון של אותו משתמש.

### א. הגדרת Gmail פר-משתמש (בתוך האפליקציה)

כל משתמש שרוצה לקבל תזכורות במייל:

1. ב-Google Account → **Security** → ודא ש-**2-Step Verification** פעיל
2. עבור ל-https://myaccount.google.com/apppasswords → צור App Password בשם "Got Schooled" → העתק את 16 התווים
3. באפליקציה → דף הגדרות → סקציית "הגדרת Gmail"
4. הזן את כתובת ה-Gmail ואת ה-App Password → לחץ **שמור ובדוק**
5. ✓ ירוק = החיבור תקין ונשמר. ✗ אדום = שגיאה (סיסמה לא נכונה / 2FA לא פעיל)

> **חשוב:** אסור להזין את הסיסמה הרגילה של החשבון — רק App Password (16 תווים). חשבונות עם 2-Step Verification הם היחידים שיכולים לייצר App Passwords.
> Gmail מגביל ל-500 מיילים ביום פר-חשבון — מעל ומעבר למספיק לשימוש אישי.

### ב. Service Role Key של Supabase

API ה-cron צריך לעקוף RLS כדי לקרוא deadlines + Gmail credentials של כל המשתמשים. זה דורש את **Service Role Key** של Supabase (לעולם לא ב-frontend!).
...
> **שים לב:** אין יותר env vars של Resend / GMAIL_*. ה-credentials של Gmail נשמרים ב-DB פר-משתמש.
...
```
אמור להחזיר `{"ok": true, "sent": N, ...}`. אם משתמש לא הגדיר Gmail בדף ההגדרות, התזכורת שלו תידלג עם `error: 'no gmail configured'` ב-results.
...
- כל משתמש שולח דרך ה-Gmail שלו (`settings.gmail_user`), ושולחים אך ורק אם יש לו `gmail_user` + `gmail_app_password` בהגדרות.
- מיילים נבחרים לפי `user_id` של ה-deadline ושולפים את המייל היעד מ-`profile.email`.
...
| תזכורות במייל לא נשלחות | (1) `curl /api/send-reminders` ידנית כדי לראות שגיאות. (2) ודא ש-`SUPABASE_URL` ו-`SUPABASE_SERVICE_ROLE_KEY` מוגדרים ב-Vercel. (3) ודא שהמשתמש הגדיר Gmail בדף ההגדרות (✓ ירוק). (4) `error: 'no gmail configured'` ב-results = המשתמש לא הגדיר Gmail. (5) `Invalid login` = App Password שגוי. (6) בדוק שהמיילים לא הולכים לספאם. |
